...
const log = createLogger("webhook:flutterwave");
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!result.handled) {
      log.warn("Flutterwave webhook signature invalid", { error: result.error });
    ...
    if (result.orderId && result.status === "paid") {
      ...
      if (order) {
        ...
        packagingService.packagePack(order.packId).catch((err: any) => {
          log.error("Flutterwave packaging failed", { error: String(err), packId: order.packId });
        });
    ...
    log.error("Flutterwave webhook error", { error: String(err) });
